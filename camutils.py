...
def reconstruct(imprefixL,imprefixR,threshold,camL,camR,colors):
    ...
    cImgL = plt.imread(colors[1])
    colImgL = np.mean(cImgL,axis=2)
    
    bImgL = plt.imread(colors[0]) 
    backGrImgL=  np.mean(bImgL,axis=2)
    maskcL = (colImgL - backGrImgL) > threshold

       
    cImgR = plt.imread(colors[3])
    colImgR = np.mean(cImgR,axis=2)
    
    bImgR = plt.imread(colors[2])
    backGrImgR = np.mean(bImgR,axis=2)
    maskcR = (colImgR - backGrImgR) > threshold
    ...
    CL = CLh + 1024*CLv
    maskL = maskLh*maskLv*maskcL
    CR = CRh + 1024*CRv
    maskR = maskRh*maskRv*maskcR
    ...
    for i in range(pts2L.shape[1]):
        cpixL.append(cImgL[pts2L[1][i]][pts2L[0][i]])
        cpixR.append(cImgR[pts2R[1][i]][pts2R[0][i]])
    
    cpix= 0.5 * (np.array(cpixR)+np.array(cpixL))

    return pts2L,pts2R,pts3,cpix.T
